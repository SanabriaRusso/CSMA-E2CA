	...
	if(argc < 12) 
	{
		if(argv[1])
		{
			...
			if((word.compare(help) == 0) || (word.compare(helpShort) == 0)){
				...
				cout << "4) Bandwidth: number of bits per second generated by the source. With 802.11n and DCF, 22e6 < is considered an unsaturated environment. Use 1000e6 for saturation under full CSMA/ECA" << endl;
